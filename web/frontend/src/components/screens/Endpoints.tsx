...
import type { EndpointOut, EndpointTestResult } from '../../lib/types';
...
export default function Endpoints(props: EndpointsProps): React.JSX.Element {
  ...
  const [form, setForm] = useState({ name: '', type: 'llama.cpp', alias: '', url: '', key: '' });
  ...
  const saveEp = useCallback(async () => {
    ...
    try {
      ...
      log.info('endpoints.create', `${form.name}${form.alias ? ` (alias ${form.alias})` : ''}`);
      setAddOpen(false);
      setForm({ name: '', type: 'llama.cpp', alias: '', url: '', key: '' });
      refresh();
    } catch {
