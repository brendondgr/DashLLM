...
// ---- auth ------------------------------------------------------------
/** GET /auth/me, POST /auth/login. `csrf` mirrors the readable cookie; the
 * session token itself is HttpOnly and never reaches JS. */
export interface MeOut {
  kind: 'admin' | 'user';
  username: string | null;
  user_id: string | null;
  private: boolean;
  /** First 10 chars of the user's rk_ key. The key itself is shown once, at
   * signup or rotation, and is not recoverable afterwards. */
  api_key_prefix: string | null;
  csrf: string | null;
}

/** GET /auth/status — the one call the login screen may make anonymously. */
...
/** POST /auth/signup, POST /auth/me/key. Returned exactly once. */
