...
def _make_user(app, username="alice"):
    """Create an account directly against the store — signup needs an invite
    code, and these tests are about the /v1 plane, not the signup flow."""
    row, key = app.state.users.create(username, "hunter2hunter2")
    return row["id"], key


def test_valid_user_key_attributes_the_request(proxy_env):
    client, app, calls = proxy_env
    _register(client, "good", "http://good/v1")
    user_id, key = _make_user(app)

    r = client.post("/v1/chat/completions",
                    headers={"Authorization": f"Bearer {key}"},
                    json={"model": "m", "messages": [{"role": "user", "content": "hi"}]})
    assert r.status_code == 200
    assert _wait_rows(app, 1)[0]["user_id"] == user_id


def test_unknown_key_is_silently_unattributed(proxy_env):
    """A bad key must not fail the request — it just lands in the general
    population, which also denies an attacker a key-probing oracle."""
    client, app, calls = proxy_env
    _register(client, "good", "http://good/v1")
    _make_user(app)

    r = client.post("/v1/chat/completions",
                    headers={"Authorization": "Bearer rk_totally-made-up"},
                    json={"model": "m", "messages": [{"role": "user", "content": "hi"}]})
    assert r.status_code == 200
    assert _wait_rows(app, 1)[0]["user_id"] is None


def test_missing_key_is_unattributed(proxy_env):
    client, app, calls = proxy_env
    _register(client, "good", "http://good/v1")
    r = client.post("/v1/chat/completions",
                    json={"model": "m", "messages": [{"role": "user", "content": "hi"}]})
...
def test_disabled_users_key_stops_attributing(proxy_env):
    client, app, calls = proxy_env
    _register(client, "good", "http://good/v1")
    user_id, key = _make_user(app)
    app.state.users.set_disabled(user_id, True)
...
def test_credentials_in_the_body_are_never_stored_or_forwarded(proxy_env):
    """The failure this guards against is specific: bodies are zlib-persisted
    to request_bodies and re-serialized to the upstream, so a client sending
    {"user_pass": ...} would write its password to disk and ship it to every
    model server in the pool."""
    client, app, calls = proxy_env
    _register(client, "good", "http://good/v1")
    client.put("/admin/settings", json={"log_bodies": True})
    ...
    r = client.post("/v1/chat/completions", json={
        "model": "m",
        "user_pass": {"username": "alice", "password": "hunter2hunter2"},
        "messages": [{"role": "user", "content": "hi"}]})
    assert r.status_code == 200

    stored = _wait_bodies(app, 1)[0]
    blob = (body_unpack(stored["prompt"]) or "") + (
        body_unpack(stored["completion"]) or "")
    assert "hunter2hunter2" not in blob and "user_pass" not in blob

    forwarded = json.dumps(calls["last_body"])
    assert "hunter2hunter2" not in forwarded and "user_pass" not in forwarded


def test_client_key_is_not_forwarded_upstream(proxy_env):
    client, app, calls = proxy_env
    _register(client, "good", "http://good/v1")
    _, key = _make_user(app)
    client.post("/v1/chat/completions",
                headers={"Authorization": f"Bearer {key}"},
                json={"model": "m", "messages": [{"role": "user", "content": "hi"}]})
    assert key not in (calls["last_auth"] or "")


def test_require_client_key_accepts_a_user_key(proxy_env, cfg):
    client, app, calls = proxy_env
    _register(client, "good", "http://good/v1")
    _, key = _make_user(app)
    app.state.cfg.require_client_key = True
    try:
        body = {"model": "m", "messages": [{"role": "user", "content": "hi"}]}
        assert client.post("/v1/chat/completions", json=body).status_code == 401
        assert client.post(
            "/v1/chat/completions", json=body,
            headers={"Authorization": "Bearer rk_wrong"}).status_code == 401
        assert client.post(
            "/v1/chat/completions", json=body,
            headers={"Authorization": f"Bearer {key}"}).status_code == 200
        assert client.post(
            "/v1/chat/completions", json=body,
            headers={"Authorization": f"Bearer {app.state.settings.api_key}"}
        ).status_code == 200
    finally:
        app.state.cfg.require_client_key = False
